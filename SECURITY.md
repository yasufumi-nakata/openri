...
- Reverse proxies should enforce upload and rate limits before OpenRI. If `X-Forwarded-For` is trusted, the proxy must strip spoofed inbound forwarding headers and uvicorn should use `--forwarded-allow-ips` scoped to trusted proxy addresses; otherwise prefer API-key-based rate limiting with `OPENRI_REQUIRE_API_KEY=true`. OpenRI only buckets on `X-OpenRI-API-Key` after that key is validated.
