...
スキャン画像だけのPDFや本文抽出できないPDFは、本文系の検査は走らず、`pdf_hidden_text` が「OCRが必要」というPDF構造リスク(coverage blocker)として報告します。OCRを先に通すか、本文テキストも併せて入力してください。
...
- `t(58) = 2.15, p = 0.003`、`χ²(2) = 1.00, p < .001`、`r(58) = .10, p < .05`、`z = 1.00, p < .001` のような報告p値を再計算し、実際のp値との差や有意性判定の反転を示します。
...
- placeholder DOIや参考文献セクション不足、本文中引用番号と参考文献番号の対応漏れを検出します。
- `(Smith, 2020)` のような著者-年引用に対応する参考文献entryがない場合を検出します。
- 主要claimが局所的な統計・引用・図表・限界記述で支えられていない場合にrisk flagを付けます。
- `Ignore previous instructions` のようなLLM査読操作の隠し指示を検出します。
- PDF内の白色文字・極小フォント・ページ外テキストに加え、注釈・埋め込みファイル・JavaScript・OCR必要性などのPDF構造リスクを検出します。
- 画像ファイルではEXIFの編集ソフト痕跡(Photoshop等)と重複領域候補をreviewer taskとして出します。
